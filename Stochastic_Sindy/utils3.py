...
# -*- coding: utf-8 -*-
"""
Created on Thu May 19 18:42:45 2022
...
"""
def library(xt, uv, polyn, modfun, harmonic):
    if polyn == 0:
        polyn = 1
                    ...
                    for l in  range(k,len(xt)):
                        for m in  range(l,len(xt)):
                            for n in  range(m,len(xt)):
                                ...
                                new = np.vstack(new)
                                D = np.append(D, new, axis=1) 
    if modfun == 1:# for the signum or sign operator
        for i in range(len(xt)):
            ind = ind+1
    ...
    # The force u
    ind = ind+1
    D = np.append(D, np.vstack(uv), axis=1)    
    # D = D[:, 1:]
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
def sparse(xdts, ydata, uv, polyorder, modfun, harmonic, MCMC, burn_in):
    # Library creation:
    D, nl = library(ydata, uv, polyorder, modfun, harmonic)

    # Adding 5% of the std. of acceleration as noise:
    xdts = xdts + 0.01*np.random.normal(0, np.std(xdts), len(xdts))
    ...
    return zstoredrift, Zmeandrift, thetadrift, mutdrift, sigtdrift, D
