...
*** This code is for the Example 1: Black-Scholes equation.
...
xdt, xbt, y, t_eval = utils_response.blackscholes(x1, T)
...
plt.subplot(223)
ax=sns.distplot(thetadrift[1,:], kde_kws={"color": "None"},  hist_kws={"color": "r"})
plt.xlabel('Drift- 'r'$\theta (x)$', fontweight='bold'); 
plt.grid(True); plt.xlim(-2, 4)
plt.xticks(fontweight='bold'); plt.yticks(fontweight='bold');
plt.title('(c)', fontweight='bold')

plt.subplot(224)
ax=sns.distplot(thetadiff[2,:], kde_kws={"color": "None"},  hist_kws={"color": "r"})
